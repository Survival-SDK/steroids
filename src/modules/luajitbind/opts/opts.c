...
st_moddata_t *st_module_luajitbind_opts_init(st_modsmgr_t *modsmgr) {
    return st_moddata_new("luajitbind", "opts", ST_MODULE_TYPE, mod_prereqs,
     st_luajitbind_init, modsmgr);
}

#ifdef ST_MODULE_TYPE_shared
st_moddata_t *st_module_init(st_modsmgr_t *modsmgr) {
    return st_module_luajitbind_opts_init(modsmgr);
}
#endif
...
static bool st_luajitbind_bind(st_luajitbindctx_t *luajitbind_ctx,
 const char *state_name) {
    ...
    return ST_LUAJITSTATE_CALL(state, run_named_string,
     "src/modules/luajitbind/opts/embedded.luajit", EMBEDDED_LUAJIT);
}
...
static st_luajitbindctx_t *st_luajitbind_init(const st_param_t params[]) {
    ...
    luajitbind_ctx = (st_luajitbindctx_t *)st_modctx_new(
     "luajitbind", "opts", sizeof(st_luajitbindctx_t), NULL,
     &luajitbindctx_funcs, (st_object_dtor_t)st_luajitbind_quit);

    if (!luajitbind_ctx) {
        ST_LOGGERCTX_CALL(logger_ctx, error,
         "luajitbind_opts: unable to create new binding context of "
         "opts utilities for luajit");
    ...
    ST_LOGGERCTX_CALL(logger_ctx, info,
     "luajitbind_opts: Opts utilities binding for luajit initialized");
...
static void st_luajitbind_quit(st_luajitbindctx_t *luajitbind_ctx) {
    ST_LOGGERCTX_CALL(luajitbind_ctx->logger_ctx, info,
     "luajitbind_opts: Opts utilities binding for luajit destroyed");
    free(luajitbind_ctx);
}
