...
deps/ - список зависимостей для сборки под разные платформы. Используется для генерации dockerfile

doc/ - пользовательская документация

dockerfiles/ - dockerfile'ы образов, в которых собирается проект. Dockerfile'ы не пишутся с нуля, а генерируются скриптом scripts/build_dockerfile.py
